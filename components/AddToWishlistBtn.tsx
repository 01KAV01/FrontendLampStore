...
const AddToWishlistBtn = ({ product, slug }: AddToWishlistBtnProps) => {
  ...
  const addToWishlistFun = async () => {
    ...
    if (session?.user?.email) {
      ...
      fetch(`http://${process.env.NEXT_PUBLIC_API_URL}/api/users/email/${session?.user?.email}`, {
        cache: "no-store",
      })
        .then((response) => response.json())
        .then((data) =>
          fetch("http://${process.env.NEXT_PUBLIC_API_URL}/api/wishlist", {
            method: "POST",
            headers: {
  ...
  const removeFromWishlistFun = async () => {
    if (session?.user?.email) {
      ...
      fetch(`http://${process.env.NEXT_PUBLIC_API_URL}/api/users/email/${session?.user?.email}`, {
        ...
        .then((data) => {
          return fetch(
            `http://${process.env.NEXT_PUBLIC_API_URL}/api/wishlist/${data?.id}/${product?.id}`,
            {
              method: "DELETE",
            }
          );
