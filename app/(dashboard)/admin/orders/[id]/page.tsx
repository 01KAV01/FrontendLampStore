...
const AdminSingleOrder = () => {
  ...
  useEffect(() => {
    const fetchOrderData = async () => {
      const response = await fetch(
        `http://${process.env.NEXT_PUBLIC_API_URL}/api/orders/${params?.id}`
      );
    ...
    const fetchOrderProducts = async () => {
      const response = await fetch(
        `http://${process.env.NEXT_PUBLIC_API_URL}/api/order-product/${params?.id}`
      );
  ...
  const updateOrder = async () => {
    if (
      ...
      fetch(`http://${process.env.NEXT_PUBLIC_API_URL}/api/orders/${order?.id}`, {
        method: "PUT", // or 'PUT'
        headers: {
  ...
  const deleteOrder = async () => {
    ...
    fetch(
      `http://${process.env.NEXT_PUBLIC_API_URL}/api/order-product/${order?.id}`,
      requestOptions
    ).then((response) => {
      fetch(
    ...
    <div className="bg-white flex justify-start max-w-screen-2xl mx-auto xl:h-full max-xl:flex-col max-xl:gap-y-5">
      ...
      <div className="flex flex-col gap-y-7 xl:ml-5 w-full max-xl:px-5">
        ...
        <div>
          <label className="form-control w-full max-w-xs">
            <div className="label">
              <span className="label-text">Компания (опционально):</span>
            </div>
            <input
              type="text"
              className="input input-bordered w-full max-w-xs"
        ...
        <div className="flex gap-x-2 max-sm:flex-col">
          ...
          <div>
            <label className="form-control w-full max-w-xs">
              <div className="label">
                <span className="label-text">Апартамент, улица, и т.д. :</span>
              </div>
              <input
                type="text"
                className="input input-bordered w-full max-w-xs"
        ...
        <div className="flex gap-x-2 max-sm:flex-col">
          <div>
            <label className="form-control w-full max-w-xs">
              <div className="label">
                <span className="label-text">Населённый пункт:</span>
              </div>
              <input
                type="text"
                className="input input-bordered w-full max-w-xs"
        ...
        <div>
          <label className="form-control">
            <div className="label">
              <span className="label-text">Пожелания к заказе:</span>
            </div>
            <textarea
        ...
        <div>
          {orderProducts?.map((product) => (
            <div className="flex items-center gap-x-4" key={product?.id}>
              ...
              <div>
                ...
                <p>
                  ₽{product?.product?.price} * {product?.quantity} предметы
                </p>
              </div>
            </div>
          ))}
          <div className="flex flex-col gap-y-2 mt-10">
            ...
            <p className="text-2xl">Перевозки: ₽5</p> */}
            <p className="text-3xl font-semibold">
              Стоимость заказа: ₽{order?.total}
              {/* Стоимость заказа: ₽{order?.total + order?.total / 5 + 5} */}
            </p>
          </div>
