...
const DashboardProductDetails = ({
  ...
  const deleteProduct = async () => {
    ...
    fetch(`http://${process.env.NEXT_PUBLIC_API_URL}/api/products/${id}`, requestOptions)
      .then((response) => {
  ...
  const uploadFile = async (file: any) => {
    ...
    try {
      const response = await fetch("http://${process.env.NEXT_PUBLIC_API_URL}/api/main-image", {
        method: "POST",
        body: formData,
      });
  ...
  const fetchProductData = useCallback (async () => {
    fetch(`http://${process.env.NEXT_PUBLIC_API_URL}/api/products/${id}`)
    .then((res) => res.json())
    .then((data) => setProduct(data));

    const imagesData = await fetch(`http://${process.env.NEXT_PUBLIC_API_URL}/api/images/${id}`, {
      cache: "no-store",
    });
